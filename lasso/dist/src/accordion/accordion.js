...
function AccordionItem({ className, ...props }) {
    return (_jsx(AccordionPrimitive.Item, { "data-slot": "accordion-item", className: cn("border-b border-border last:border-b-0", className), ...props }));
}
/**
 ...
 */
function AccordionTrigger({ className, children, showArrowByName = false, hideArrow = false, ...props }) {
    return (_jsx(AccordionPrimitive.Header, { className: "flex", children: _jsxs(AccordionPrimitive.Trigger, { "data-slot": "accordion-trigger", className: cn("focus-visible:border-ring focus-visible:ring-ring/50 flex flex-1 items-start gap-2 rounded-sm py-4 text-left text-base font-medium lasso:wght-medium leading-6 text-foreground transition-all outline-none hover:underline focus-visible:ring-[3px] disabled:pointer-events-none disabled:opacity-50 [&[data-state=open]>svg]:rotate-180", className), ...props, children: [!showArrowByName && children, !hideArrow && (_jsx(CaretDown, { className: "text-muted-foreground pointer-events-none size-4 shrink-0 translate-y-0.5 transition-transform duration-200" })), showArrowByName && children] }) }));
}
/**
...
function AccordionContent({ className, children, ...props }) {
    return (_jsx(AccordionPrimitive.Content, { "data-slot": "accordion-content", className: "data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down overflow-hidden text-sm font-normal lasso:wght-normal leading-5 text-muted-foreground", ...props, children: _jsx("div", { className: cn("pt-0 pb-4", className), children: children }) }));
}
