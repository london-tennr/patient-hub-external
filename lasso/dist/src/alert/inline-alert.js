...
const InlineAlertContext = React.createContext(null);
const inlineAlertContentVariants = cva("flex items-center justify-between py-3 px-4 w-full shrink-0", {
    variants: {
        variant: {
 ...
 */
function InlineAlert({ className, variant, children, onClose, ...props }) {
    return (_jsx(InlineAlertContext.Provider, { value: { variant, onClose }, children: _jsx("div", { className: "mx-0.5", children: _jsx("div", { "data-slot": "alert", role: "alert", className: cn("relative w-full rounded-md overflow-hidden flex flex-col", className), ...props, children: children }) }) }));
}
/**
 ...
 */
function InlineAlertContent({ className, iconName, children, ...props }) {
    const context = React.useContext(InlineAlertContext);
    if (!context) {
        throw new Error("InlineAlertContent must be used within an InlineAlert component");
    }
    const { variant, onClose } = context;
    const defaultIcon = variantIcons[variant];
    const iconToRender = iconName || defaultIcon;
    return (_jsx("div", { "data-slot": "alert-header", className: cn(inlineAlertContentVariants({ variant }), className), ...props, children: _jsxs("div", { className: "flex flex-row items-center justify-between w-full gap-2", children: [_jsxs("div", { className: "flex items-start gap-2", children: [iconToRender && (_jsx("div", { className: "size-5 p-0.5", children: _jsx(Icon, { icon: iconToRender, className: "shrink-0" }) })), _jsx(Text, { as: "div", variant: "base-sm", children: children })] }), onClose && (_jsx(Button, { variant: "ghost", size: "icon", className: "hover:bg-black/10 shrink-0 w-6 h-6 self-start text-inherit hover:text-inherit", onClick: onClose, children: _jsx(Icon, { icon: "ph:x" }) }))] }) }));
}
