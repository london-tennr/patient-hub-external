...
function BreadcrumbSeparator({ children, className, ...props }) {
    return (_jsx("li", { "data-slot": "breadcrumb-separator", role: "presentation", "aria-hidden": "true", className: cn("[&>svg]:size-3.5", className), ...props, children: children ?? _jsx(CaretRight, {}) }));
}
/**
...
function BreadcrumbEllipsis({ className, children, ...props }) {
    return (_jsx("span", { "data-slot": "breadcrumb-ellipsis", role: "presentation", "aria-hidden": "true", className: cn("flex items-center justify-center select-none", className), ...props, children: children ?? "..." }));
}
/**
 ...
 * ```
 */
function BreadcrumbDropdown({ children, items, className, ...props }) {
    return (_jsxs(DropdownMenu, { children: [_jsx(DropdownMenuTrigger, { asChild: true, children: _jsx("span", { "data-slot": "breadcrumb-dropdown", className: cn("flex items-center justify-center cursor-pointer hover:bg-muted focus:bg-muted transition-colors border-none bg-transparent p-0 select-none", className), ...props, children: children ?? "..." }) }), _jsx(DropdownMenuContent, { align: "start", children: items.map((item, index) => (_jsx(DropdownMenuItem, { onClick: item.onClick, asChild: !!item.href, className: "cursor-pointer hover:bg-muted transition-colors border-none bg-transparent", children: item.href ? (_jsx("a", { href: item.href, children: item.label })) : (_jsx("span", { children: item.label })) }, index))) })] }));
}
