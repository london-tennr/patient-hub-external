...
const buttonV2Variants = cva("inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-[4px] text-sm font-medium transition-all focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 relative overflow-hidden", {
    variants: {
        variant: {
            primary: "bg-brand-terracotta text-white shadow-[0_1px_1px_rgba(0,0,0,0.2),0_2px_4px_rgba(0,0,0,0.1)] " +
            ...
            outline: "bg-gradient-to-b from-white from-[31%] to-neutral-2 to-[93%] text-text-primary " +
...
const ButtonV2 = React.forwardRef(({ className, variant, size, asChild = false, ...props }, ref) => {
    const Comp = asChild ? Slot : "button";
    return (_jsx(Comp, { className: cn(buttonV2Variants({ variant, size, className })), ref: ref, ...props }));
});
