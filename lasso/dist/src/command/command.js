...
function Command({ className, ...props }) {
    return (_jsx(CommandPrimitive, { "data-slot": "command", className: cn("bg-popover text-popover-foreground flex h-full w-full flex-col overflow-hidden rounded-sm", className), ...props }));
}
/**
...
function CommandInput({ className, wrapperClassName, ...props }) {
    return (_jsxs("div", { "data-slot": "command-input-wrapper", className: cn("flex h-9 items-center gap-2 border-b legacy:border-border px-3", wrapperClassName), children: [_jsx(MagnifyingGlass, { weight: "light", className: "size-4 shrink-0 opacity-50" }), _jsx(CommandPrimitive.Input, { "data-slot": "command-input", className: cn("placeholder:text-muted-foreground flex h-10 w-full rounded-sm bg-transparent py-3 text-sm outline-hidden disabled:cursor-not-allowed disabled:opacity-50", className), ...props })] }));
}
/**
