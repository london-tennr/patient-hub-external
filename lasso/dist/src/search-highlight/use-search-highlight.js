 ...
 */
export function useSearchHighlight(searchableContentContainer) {
    ...
    const collectTextElements = useCallback(() => {
        if (!searchableContentContainer)
            return [];
        const walker = document.createTreeWalker(searchableContentContainer, NodeFilter.SHOW_TEXT);
        const nodes = [];
    ...
    const highlightAndCollectMatches = useCallback((textElements) => {
        if (!searchTerm)
            return [];
        const newMatches = [];
