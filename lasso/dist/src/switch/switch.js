...
function Switch({ className, ...props }) {
    return (_jsx(SwitchPrimitive.Root, { "data-slot": "switch", className: cn("peer data-[state=checked]:bg-brand-peat data-[state=unchecked]:bg-muted-foreground/50 focus-visible:ring-ring/50 inline-flex h-5 w-9 px-0.5 shrink-0 items-center rounded-full border-0 shadow-xs transition-all outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50", className), ...props, children: _jsx(SwitchPrimitive.Thumb, { "data-slot": "switch-thumb", className: cn("bg-white pointer-events-none block size-4 rounded-full shadow-lg transition-transform data-[state=checked]:translate-x-4 data-[state=unchecked]:translate-x-0") }) }));
}
export { Switch };
