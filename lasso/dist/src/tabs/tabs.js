 ...
 */
function TabsList({ className, variant, size, ...props }) {
    return (_jsx(TabsPrimitive.List, { "data-slot": "tabs-list", className: cn(tabsListVariants({ variant, size }), className), ...props }));
}
/**
 ...
 */
const tabsTriggerVariants = cva("inline-flex items-center hover:cursor-pointer justify-center whitespace-nowrap transition-colors focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50", {
    variants: {
        variant: {
            default: "data-[state=active]:bg-border data-[state=active]:text-foreground data-[state=inactive]:bg-transparent data-[state=inactive]:text-muted-foreground focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-0 h-full px-2 py-1 text-sm font-medium lasso:wght-medium leading-5 rounded-sm transition-colors [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
 ...
 * @param className - Additional CSS classes to apply
 */
function TabsTrigger({ className, variant, size, ...props }) {
    return (_jsx(TabsPrimitive.Trigger, { "data-slot": "tabs-trigger", className: cn(tabsTriggerVariants({ variant, size }), className), ...props }));
}
/**
