 ...
 */
const Text = ({ className, variant, family, size, weight, mono, leading, tracking, transform, as: Component = "p", ref, ...props }) => {
    const variantClasses = variant ? textVariants[variant] : "";
    // Only apply individual classes if any of these props are provided
    const hasIndividualStyles = family || size || weight || mono || leading || tracking || transform;
    ...
    return _jsx(Component, { className: classes, ref: ref, ...props });
};
